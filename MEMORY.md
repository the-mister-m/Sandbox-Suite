...
- Where two build jobs share a file, table order wins over map order.
- Every build brief carries three token lines: no whole reads over 400 lines, a read ledger per receipt, a 60-call handoff line.

## WARM START — 2026-09-15 (Phase 3.5-Adobe, first seam)
- Situation: Phase 3.5-Adobe (HTML-only layout/vector, doc mode removed) built job 0 through W1 plus F4, eighteen subagent runs, all Goto with model overrides. Doc mode gone, contract 3.1 proven, page setup/rulers/guides/snap/zoom/snippets/layers-panel/layers-canvas built and redpenned green (R1, R2 both red then green). First seam walked headed in W1.
- Last state: W1 headed walk 3 of 8 scope-6 lines PASS. Lines 4, 13, 14 fail only on masters (job 14 unbuilt, no Master layer). Line 5 fails: cross-layer drop lands the row at the front of Art so ▲ greys. Line 6 fixed by F4 (dropped text frame now takes typing, phase35_15.py 5/5). Stopped at the seam, jobs 6 and 8 not started. Brandon to rule: W1 line 5 (scope wording vs code), scope 3.4 job 2's "set-css-token" (no such kind, shipped as set-css-rule), scope 3.3 activeLayer "of matching plugin" clarification from F3, whether W1 lines 4/13/14 stay in W1 or move to W3.
- Next move: jobs 6 and 8 together, then 7, then 9, 10, R3, W2; then 11 and 13 together, 12, R4, 14, then 17 beside W3. Session id for the headless harnesses is 0d78d246515f; server was already running the whole session.
- Links: [SESSION-REVIEW-2026-09-14-phase3.5-adobe-build.md](Docs/Reports/SESSION-REVIEW-2026-09-14-phase3.5-adobe-build.md), [SCOPE-phase3.5-adobe.md](Docs/Scope/Code%20Canvas%20port/SCOPE-phase3.5-adobe.md), [SESSIONLOG.md](SESSIONLOG.md), [TODO.md](TODO.md).

## WARM START — 2026-09-14 (Live Actions P1-P3)
- Situation: gated Claude-rail actions showed as two unpaired rows and chat/ledger stayed silent mid-turn; P1 gate merge, P2 chat live actions, P3 ledger live scoped and built by three Sonnet Gotos in parallel, every diff line reviewed by the session agent.
- Last state: built, not restarted, not browser-tested. P1: Claude gate + tool record merge into one row (`_pair_claude_gates`), `merge_gates` toggle default on on queue/queue-log/mini-queue, native pairing unchanged; mini-queue's toggle is wired but inert (pending-only view). P2: server pushes live tool cards via `region.hub.tool` on gate decision and result; anchor-chat updates cards in place by id; approve/deny inside the card reuses gate-common.js, no new frame. P3: ledger shows a live turn row (region+turn keyed, open-by-default, collapses once), Write/Edit sub-rows show a real diff, Bash sub-rows show command+output.
- Next move: restart the server once, browser-test all three (P1 receipt's "no restart needed" was wrong — server.py:2325 runs without reloader). P4 tonight: per-call $ mid-turn, rows collected at providers.py:699 never sent, ledger `_call` never filled. Two leads found mid-session, not fixed: ledger transcript fetched once per region and never refreshes (fix drafted, not applied) — ledger.js:628-630; regions closed before a session is saved lose their transcript — tracks.py:1448/1750, `remove_track` callers not traced.
- Links: [SESSION-REVIEW-2026-09-14-live-actions.md](Docs/Reports/SESSION-REVIEW-2026-09-14-live-actions.md), [RECEIPT-live-actions-P1.md](Docs/Reports/RECEIPT-live-actions-P1.md) / [-P2](Docs/Reports/RECEIPT-live-actions-P2.md) / [-P3](Docs/Reports/RECEIPT-live-actions-P3.md), [SESSIONLOG.md](SESSIONLOG.md), [TODO.md](TODO.md).

## WARM START — 2026-09-14 (Code Canvas skills + fixes)
- Situation: two skills written to injections/skills/ (codecanvas-howto.md, codecanvas-agent.md), kept current with the build; SPEC-codecanvas-fixes.md's A1-A3, B1-B2 plus addendum A1b built.
- Last state: all five fixes plus A1b syntax-checked clean, no headed browser run this session. Group geometry held out of scope, open pending Brandon's play.
- Next move: headed pass against the spec's six acceptance checks; Brandon plays with Group in file mode then rules bounding-box geometry vs selection-alias; open the two skills once as the user would and flag what's wrong to fable.2; decide whether border stays in the inspector's Border group now its three parts are separate props.
- Links: [SESSION-REVIEW-2026-09-14-codecanvas-skills.md](Docs/Reports/SESSION-REVIEW-2026-09-14-codecanvas-skills.md), [RECEIPT-codecanvas-fixes.md](Docs/Reports/RECEIPT-codecanvas-fixes.md), [SPEC-codecanvas-fixes.md](Docs/Specs/SPEC-codecanvas-fixes.md), [SESSIONLOG.md](SESSIONLOG.md).
