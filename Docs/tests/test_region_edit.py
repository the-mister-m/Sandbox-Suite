...
def _make_region(environment, reset_on_change):
    track = tracks.create_track("edit test", root=".", environment=environment)
...
def test_setting_edit_with_reset_on_change_replaces_the_region(environment):
    ...
    assert any(row.get("id") == old_id for row in tracks.closed_rows(environment))


def test_setting_edit_without_reset_on_change_stays_in_place(environment):
...
def test_rename_never_resets(environment):
    for flag in (True, False):
        track, region = _make_region(environment, flag)
        old_id = region.id
...
def test_editing_reset_on_change_itself_applies_in_place(environment):
    track, region = _make_region(environment, True)
    old_id = region.id
...
def test_cloud_model_defaults_reset_on_change_on(environment):
    track = tracks.create_track("cloud test", root=".", environment=environment)
    region = tracks.insert_region(track.id, "cloud test", "sonnet", environment=environment)
    assert region.sess.settings["reset_on_change"] is True


def test_local_model_defaults_reset_on_change_on(environment):
    track = tracks.create_track("local test", root=".", environment=environment)
    region = tracks.insert_region(track.id, "local test", "gemma4:26b", environment=environment)
    assert region.sess.settings["reset_on_change"] is True


def test_index_entry_carries_the_full_bag(environment):
    track, region = _make_region(environment, False)
