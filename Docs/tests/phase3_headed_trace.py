...
def wire(page, tag):
    ...
    # trace: name + stack on every pageerror, not only str(e)
    page.on("pageerror", lambda e: (
        PAGEERRORS.append(f"[{tag}:pageerror] {e}"),
        CONSOLE.append(f"[{tag}:pageerror] {e}"),
        CONSOLE.append(f"[{tag}:pageerror:name] {getattr(e, 'name', '?')}"),
        CONSOLE.append(f"[{tag}:pageerror:stack] {getattr(e, 'stack', '?')}")))
    # trace: rejection stacks reach console.txt; survives navigation
    page.add_init_script(
        'window.addEventListener("unhandledrejection", ev => '
        'console.log("[trace] " + (ev.reason && ev.reason.stack || ev.reason)));')
