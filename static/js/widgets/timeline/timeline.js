...
.mx-timeline #tlHeads .ruler-pad{
  height:28px; border-bottom:1px solid var(--gridline);
  position:sticky; top:0; z-index:6; background:var(--surface-1);
  display:flex; align-items:center; gap:14px; padding:0 9px;
}
...
.mx-timeline .tl-zoom{ display:flex; align-items:center; gap:4px; }
...
`;
  ...
  function render(frame) {
    ...
    const lanes = trackRows
      ...
      .concat(tl.showEnded ? unattributedDeadIds.map((id) =>
        ({ id, container: null, name: nameOf(tl, id), reg_name: "no track",
    ...
    lanes.forEach((tr) => {
      ...
      h.appendChild(top);
      if (tr.track) {
        if (isClaudeModel(tr.track.model)) top.appendChild(cacheTtlToggle(frame, tr.track));
        if (isClaudeModel(tr.track.model)) top.appendChild(excludeDynamicToggle(frame, tr.track));
      }
  ...
  MX.registerWidget("timeline", {
    mount(frame) {
      ...
      MX.mountAddControls(headActions, frame, { mode: "track" });
      MX.mountAddControls(headActions, frame, { mode: "region", track: () => tl.lastTrackId || null });
