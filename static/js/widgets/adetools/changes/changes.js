...
`;
  ...
  function reduceEvents(records) {
    ...
    for (const r of records) {
      if (r.action_type === "write" || r.action_type === "write_file") {
  ...
  function nameOf(c, region) {
    ...
    const closed = c.names[region];
    return closed === undefined ? region : closed;
  }
  ...
  MX.registerWidget("changes", {
    mount(frame) {
      ...
      const c = frame._changes = {
        ...
        selected: undefined, trackNames: {}, names: {}, feedDirtyTimer: null,
      };
    ...
    unmount(frame) {
      const c = frame._changes;
      if (c && c.feedDirtyTimer) clearTimeout(c.feedDirtyTimer);
      frame._changes = null;
    },
    ...
    onFrame(frame, msg) {
      ...
      if (msg.type === "track_list" || msg.type === "ade_init") {
        for (const row of (msg.tracks || [])) {
          if (row && row.id !== undefined) c.trackNames[row.id] = row.name !== undefined ? row.name : row.id;
        }
        c.names = Object.assign({}, c.names, msg.names || {});
        render(frame);
        return;
      }
      if (msg.type === "feed_dirty") {
        if (c.feedDirtyTimer) clearTimeout(c.feedDirtyTimer);
        c.feedDirtyTimer = setTimeout(() => {
          c.feedDirtyTimer = null;
          frame.send({ type: "feed", inst: frame.id });
        }, 1000);
        return;
      }
