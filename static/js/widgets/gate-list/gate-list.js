  ...
  MX.registerWidget('gate_list', {
    ...
    mount(frame) {
      ...
      g.render = () => {
        _renderGateListInto(g.el, g.gates, () => {}, (gateId, action) => {
          ...
          const found = outcome && g.gates.find(x => x.id === gateId);
          if (found) {
            found.active = false;
            found.settled = true;
            found.outcome = outcome;
            found.settledAt = ++_settleSeq;
            g.gates = _capSettledGates(g.gates);
            g.render();
          }
          const out = { type: 'answer', text: _ALERT_GATE_TEXT[action] || 'n', id: gateId };
          if (g.region) out.track = g.region;
          frame.send(out);
        });
      };
      g.render();

      frame.subscribe(['ask', 'gate_pending', 'chat_history', 'region_replaced']);
    ...
    onFrame(frame, msg) {
      ...
      switch (msg.type) {
        case 'gate_pending': {
          const next = [];
          if (msg.active) next.push({ id: msg.active.id, prompt: msg.active.prompt, active: true });
          (msg.pending || []).forEach(p => next.push({ id: p.id, prompt: p.prompt, active: false }));
